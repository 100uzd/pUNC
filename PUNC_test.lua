...
local function script()
    ...
    -- Classify the function based on its type and characteristics
    local function classifyFunction(func, name)
        ...
        -- Detect upvalue count (for deeper inspection)
        for _ in pairs(upvals) do
            upvalCount = upvalCount + 1
        end
    
        -- Determine function status based on debug info and other checks
        ...
        elseif info then
            -- Check for more specific function types based on info
            if info.what == "C" then
        ...
        -- Check for "getrawmetatable" presence to determine if hook status can be checked
        local isHooked = "unknown"
        if type(getrawmetatable) == "function" then
            -- Check if function is hooked or replaced with a custom handler
        ...
        -- Final classification result with hook awareness
    ...
    -- Add function securely to addTests
    ...
    -- Intermediate Loadstring addTest (more complex Lua code)
    ...
    -- Strict Loadstring addTest (addTesting restrictions)
    addTest("strict_loadstring", function()
        local env = { getgenv = nil }  -- Simulating a restricted environment
        local chunk = loadstring("return getgenv")  -- Should be restricted
        setfenv(chunk, env)
        local result = pcall(chunk)
        assert(not result, "strict_loadstring failed, should be restricted")
    end)
    
    -- Heavy Loadstring addTest (loading large Lua chunk)
    addTest("heavy_loadstring", function()
        local largeCode = "return " .. string.rep("1 + 1;", 1000)  -- Generate a large Lua code block
    ...
    -- Other standard addTests (keeping existing ones)
    ...
    addTest("http_request", function()
        ...
        -- Assert the response is a table
        assert(type(response) == "table", "Response must be a table")
        ...
        local data = game:GetService("HttpService"):JSONDecode(response.Body)
        -- Assert the data is a table and contains the user-agent key with a string value
        assert(type(data) == "table" and type(data["user-agent"]) == "string", "Did not return a table with a user-agent key")
    ...
    -- ===============================================================
    --                   🔒 Hardened Security addTests
    -- ===============================================================
    local function assertEqual(actual, expected, message)
        assert(actual == expected, message .. " (Expected: " .. tostring(expected) .. ", Got: " .. tostring(actual) .. ")")
    end
    
    -- Add addTest for getgenv
    addTest("getgenv", function()
        -- Check if getgenv is available and can access global environment
    ...
    addTest("hookmetamethod", function()
        ...
        -- Hook the __index metamethod for a table
        local myTable = {}
    ...
    addTest("setreadonly", function()
        ...
        -- addTest that nested tables are protected
        ...
        -- addTest the immutability of the readonly state
    ...
    addTest("getgc", function()
        ...
        -- Get the global garbage collector list and check if it's a table
        local gcTable = getgc()
        assert(type(gcTable) == "table", "getgc should return a table")
        
        -- Ensure that the table contains instances or userdata
        local found = false
    ...
    addTest("gethiddenproperty", function()
        ...
        -- Ensure the property is hidden (should return true)
        assertEqual(isHidden, true, "Property 'size_xml' should be marked as hidden, but it's not")
        
        -- addTest with a property that doesn't exist to check edge case
    ...
    addTest("getscripthash", function()
        ...
        -- addTest getscripthash with a running script (using current script)
    ...
    addTest("checkcaller", function()
        -- Ensure checkcaller exists and is callable
        assert(type(checkcaller) == "function", "checkcaller is missing or not a function")
    
        -- addTest with a valid caller (inside the same function)
        local function addTestCaller()
            assert(checkcaller(), "checkcaller failed for a valid call")
        end
    
        addTestCaller()  -- This should pass since we're calling it within the same function
    
        -- addTest for a false case (calling from a different context would likely fail)
    ...
    addTest("clonefunction", function()
        -- Ensure clonefunction exists and is callable
    ...
    addTest("getscriptclosure", function()
        ...
        -- Check if a known constant is present and matches in both the original and generated tables
    ...
    addTest("cloneref", function()
        -- Ensure cloneref exists and is callable
        ...
        -- Ensure the reference was cloned correctly (should not be the same object)
        assert(original ~= cloned, "cloneref did not create a new reference")
        
        -- Ensure the properties match between original and cloned
        assertEqual(cloned.Size, original.Size, "Cloned reference has incorrect properties")
    end)
    
    -- compareinstances addTest with improvements
    addTest("compareinstances", function()
        -- Ensure compareinstances exists and is callable
    ...
    -- readfile addTest with improvements
    addTest("readfile", function()
        -- Ensure readfile exists and is callable
        assert(type(readfile) == "function", "readfile is missing or not a function")
    
        -- Prepare a addTest file
        ...
        -- Clean up the file after addTest
        delfile(filename)
    end)
    
    -- listfiles addTest with improvements
    addTest("listfiles", function()
        -- Ensure listfiles exists and is callable
        assert(type(listfiles) == "function", "listfiles is missing or not a function")
    
        -- Prepare a addTest folder
    ...
    -- makefolder addTest with improvements
    addTest("makefolder", function()
        -- Ensure makefolder exists and is callable
        assert(type(makefolder) == "function", "makefolder is missing or not a function")
    
        -- addTest folder creation
        local folderName = "addTestFolder"
        makefolder(folderName)
    
        -- Verify folder creation (check if it exists)
        local success, err = pcall(function()
            listfiles(folderName)
        end)
        assert(success, "makefolder failed to create the folder")
    
        -- Clean up the folder after addTest
        delfile(folderName)
    end)
    
    -- appendfile addTest with improvements
    addTest("appendfile", function()
        -- Ensure appendfile exists and is callable
        assert(type(appendfile) == "function", "appendfile is missing or not a function")
    
        -- addTest appending to a file
    ...
    -- delfile addTest with improvements
    addTest("delfile", function()
        -- Ensure delfile exists and is callable
        assert(type(delfile) == "function", "delfile is missing or not a function")
    
        -- Create a addTest file
    ...
    -- dofile addTest with improvements
    addTest("dofile", function()end)
    
    -- isrbxactive addTest with improvements
    addTest("isrbxactive", function()
        -- Ensure isrbxactive exists and is callable
    ...
    addTest("WebSocket.connect", function()
        ...
        -- addTest WebSocket.connect() with valid URL
        ...
        -- addTest WebSocket.connect() with invalid URL
    ...
    for name, fn in pairs(addTests or {}) do  -- Handle nil addTests table
        local success, result = pcall(fn)
        if success then
            -- Capture addTest output or use default message
            local output = result or "Passed"
            table.insert(passed, string.format("✅ %s: %s", name, output))
        else
            -- Handle nil error messages safely
    ...
    -- Print passed addTests
    if #passed > 0 then
        print("\n[ PASSED Functions ]")
        print("-----------------------------------------------------")
        for _, pass in ipairs(passed) do
            -- Extract addTest name and output from the formatted string
